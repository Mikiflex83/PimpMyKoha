/*
 ...
 Koha : 25.11+
 Version canonique KohaTools v1.2
 Référence métier importée : v10 bibliothèque (en-tête interne v9)
...
(() => {
    ...
    const MODULE_ID = 'temporary-lists';
    const CONFIG = KT.Config.getCanonical(MODULE_ID);
    const FirebaseModule = KT.getService && KT.getService('firebase-module');
    const Scope = KT.getService && KT.getService('scope');
    const Sidebar = KT.getService && KT.getService('sidebar');

    if (!CONFIG || !CONFIG.enabled || !['shadow','live'].includes(CONFIG.mode)) return;
    if (Scope && !Scope.match(CONFIG.general && CONFIG.general.scope).ok) return;

    if (CONFIG.mode === 'shadow') {
        const legacySignals = {
            legacyPanel: !!document.getElementById('sidebar'),
            bottomBar: !!document.getElementById('bottomActionBar'),
            searchButtons: document.querySelectorAll('.kx-temp-list-toggle, button[title="Mettre de côté"]').length,
            detailPage: window.location.pathname === CONFIG.detailPath,
            searchPage: window.location.pathname === CONFIG.searchPath
        };
        KT.record({
            module: MODULE_ID,
            level: 'info',
            kind: 'shadow-v10-reference-audit',
            reference: '066-067-lists-manager-firestore-sans-compte-v10',
    ...
    function shouldSkipPage() {
        ...
        const loginSelector = CONFIG.advanced?.skipLoginSelector || '#login';
        const pattern = CONFIG.advanced?.skipPathRegex ? new RegExp(CONFIG.advanced.skipPathRegex, 'i') : null;
        return !!document.querySelector(loginSelector) || !!(pattern && (pattern.test(href) || pattern.test(path)));
    }
    ...
    const firebaseMeta = CONFIG.firebase || {};
    const recordFirebase = (op,n=1) => { try { firebaseBudget()?.record?.(firebaseMeta.projectId,op,n,{kind:'firestore'}); } catch (_) {} };
    ...
    async function loadFirebase() {
        ...
        state.firebaseStatus = 'loading'; renderEverything();
        state.firebasePromise = (async () => {
            try {
                const meta = FirebaseModule?.require?.(MODULE_ID, firebaseMeta) || firebaseMeta;
                const publicCfg = FirebaseModule?.publicConfig?.(MODULE_ID, meta, { throwOnError: true }) || meta;
                const sdkVersion = meta.sdkVersion || CONFIG.firebaseSdkVersion || '12.18.0';
                const base = `https://www.gstatic.com/firebasejs/${sdkVersion}`;
                const [appMod, firestoreMod] = await Promise.all([import(`${base}/firebase-app.js`), import(`${base}/firebase-firestore.js`)]);
                const appName = meta.appName || CONFIG.firebaseAppName || 'kx-personal-lists';
                let app = appMod.getApps().find(candidate => candidate.name === appName);
                if (!app) app = appMod.initializeApp(publicCfg, appName);
                const db = meta.databaseId && meta.databaseId !== '(default)' ? firestoreMod.getFirestore(app, meta.databaseId) : firestoreMod.getFirestore(app); state.fb = { appMod, firestoreMod, app, db }; state.firebaseError = '';
                const savedName = sanitizeProfileName(localStorage.getItem(CONFIG.storage.profileName));
                if (savedName) await activateProfile(savedName, { silent:true, profileId:profileIdForName(savedName), registerProfile:false });
                else { state.firebaseStatus='needs-profile'; renderEverything(); }
                return state.fb;
            } catch(error) { state.firebaseStatus='error'; state.firebaseError=firebaseErrorMessage(error); renderEverything(); throw error; }
            finally { state.firebasePromise=null; }
        })();
        return state.firebasePromise;
    }
    async function ensureFirebaseStarted(){ if(state.fb)return true; try{await loadFirebase();return !!state.fb}catch(_){return false} }
    ...
    async function activateProfile(rawName, options = {}) {
        ...
        try {
            if (options.registerProfile === true) {
                ...
                await setDoc(doc(state.fb.db, 'profiles', id), { displayName, canonicalName:canonicalProfileName(displayName), updatedAt:serverTimestamp() }, { merge:true });
    ...
    async function initializeUserLists() {
        ...
        await fetchListsOnce();
        reconcileActiveLists();
        if (document.getElementById('kx-temp-lists-panel')?.classList.contains('is-open')) { subscribeLists(); subscribeEntries('notices'); subscribeEntries('items'); }
    }

    function subscribeLists() {
        if (!state.profile) return;
        if (typeof state.unsubLists === 'function') state.unsubLists();
        state.unsubListsBudget = firebaseBudget()?.listenerStart?.(firebaseMeta.projectId,'personal-lists','firestore') || null;
    ...
    function subscribeEntries(tab) {
        ...
        state.unsubEntriesBudget = state.unsubEntriesBudget || {};
        state.unsubEntriesBudget[tab] = firebaseBudget()?.listenerStart?.(firebaseMeta.projectId,`personal-list-${tab}`,'firestore') || null;
    ...
    function buildIdentityArea() {
        ...
        select.addEventListener('focus', async () => { if(!state.fb)await ensureFirebaseStarted();if(!state.fb)return;try{await refreshProfileDirectory();renderIdentityArea();}catch(_){} });
    ...
    function setPanelOpen(open, persist = true) {
        const panel=document.getElementById('kx-temp-lists-panel'); if(!panel)return;
        ...
        if(!open)stopCloudListeners(); else if(canUseCloud()){subscribeLists();subscribeEntries('notices');subscribeEntries('items');}
    }
    async function togglePanel(){const panel=document.getElementById('kx-temp-lists-panel');if(!panel)return;const opening=!panel.classList.contains('is-open');setPanelOpen(opening);if(opening){const ok=await ensureFirebaseStarted();if(ok&&state.profile){subscribeLists();subscribeEntries('notices');subscribeEntries('items');}renderEverything();}}
    ...
    const HELP_ASSIST_KEY = CONFIG.storage?.assistEnabled || 'kohaGuideAssistEnabled';
    ...
    function renderPageButtons() {
        if (CONFIG.features?.legacyUiCleanup !== false) cleanupLegacyUI();
        if (CONFIG.features?.searchIntegration !== false) integrateSearchPage();
        if (CONFIG.features?.detailIntegration !== false) integrateDetailPage();
        if (CONFIG.features?.bottomBar !== false) updateBottomBarCount();
    }
    ...
    function init() {
        ...
        if (CONFIG.features?.panel !== false) createPanel();
        if (CONFIG.features?.bottomBar !== false) ensureBottomBar();

        if (CONFIG.features?.sidebarEntry && Sidebar) {
            const spec = { ...(CONFIG.navigation?.sidebarAction || {}) };
            spec.onClick = function(event) {
                event?.preventDefault?.();
                togglePanel();
            };
            Sidebar.register(spec);
        }

        renderEverything();
        if (CONFIG.advanced?.observer !== false) observeRelevantDom();

        document.addEventListener('keydown', event => {
            if (event.key === 'Escape') setPanelOpen(false);
        });

        if (CONFIG.features?.cloudSync !== false && localStorage.getItem(CONFIG.storage.panelOpen) === 'true') { setPanelOpen(true,false); ensureFirebaseStarted().then(()=>renderEverything()); }
    }
