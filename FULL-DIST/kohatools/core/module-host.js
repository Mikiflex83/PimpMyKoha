...
function open(id){const d=descriptors.get(id);if(!d){KT.record?.({module:id,level:"warn",kind:"module-open-unregistered"});return false}if(!enabled(id)){KT.record?.({module:id,level:"info",kind:"module-open-blocked-access"});return false}if(d?.special==="dashboard"){global.KohaToolsDashboard?.open?.();return true}if(location.pathname.endsWith("/mainpage.pl")){location.hash=`kt/module/${encodeURIComponent(id)}`;mountRoute()}else location.href=url(id);return true}
