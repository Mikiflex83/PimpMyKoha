...
const canonicalFor=m=>defaults?.modules?.[m.id]?.config?.canonicalModule||m.canonicalModule||null;
const byCanonical=new Map();for(const m of manifest.modules||[]){const cid=canonicalFor(m);if(!cid)continue;if(!byCanonical.has(cid))byCanonical.set(cid,[]);byCanonical.get(cid).push(m)}
for(const [cid,maps] of byCanonical){const certified=maps.filter(m=>m?.freshInstall?.certified===true);if(!certified.length)continue;if(certified.length!==maps.length)errors.push(`Certification fresh-install partielle interdite: ${cid}`);const cc=defaults.canonicalModules?.[cid];if(!cc)errors.push(`Configuration canonique absente pour module certifié: ${cid}`);if(!cc?.recipe)errors.push(`Recette absente pour module certifié: ${cid}`);for(const m of maps){if(m?.freshInstall?.supported!==true)errors.push(`Module certifié non supporté fresh-install: ${m.id}`);if(m?.freshInstall?.requiresLegacy!==false)errors.push(`Module certifié dépend encore du legacy: ${m.id}`);if(m.legacyStillRequired!==false)errors.push(`legacyStillRequired doit être false: ${m.id}`);if(!m.nextModule||!fs.existsSync(path.join(root,m.nextModule)))errors.push(`Candidate absente pour module certifié: ${m.id}`);const dm=defaults.modules?.[m.id];if(dm?.general?.legacyStillRequired!==false)errors.push(`Defaults legacyStillRequired doit être false: ${m.id}`);if(dm?.advanced?.allowLegacyFallback!==false)errors.push(`Fallback legacy encore autorisé pour module certifié: ${m.id}`)}}
