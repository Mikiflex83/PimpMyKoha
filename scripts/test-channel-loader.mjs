...
const index={
  ...
  channels:{stable:'3.43.0',canary:'3.44.0-canary.1',dev:'3.44.0-dev.3'},
  releases:{
    ...
    '3.44.0-dev.3':{version:'3.44.0-dev.3',runtimeRoot:'https://example.test/releases/3.44.0-dev.3/'}
  }
};
...
assert.equal(r.resolution.version,'3.44.0-dev.3');
assert.equal(r.bootstrap.distributionChannel,'dev');
assert.match(r.loaded[0],/3\.44\.0-dev\.3\/bootstrap\.js$/);
...
assert.equal(r.resolution.version,'3.43.0');
...
const noDev=structuredClone(index);delete noDev.channels.dev;delete noDev.releases['3.44.0-dev.3'];
