...
function boot({certified={},blocked={},developer=false}={}){
  const services={
    ...
    "access-control":{can:role=>role==="developer"&&developer,channel:()=>developer?"dev":"stable"}
  };
...
assert.equal(
  c.modeForManifestModule('manifest-wrapper',{canonicalModule:'setup'}),
  'live',
  'le canonicalModule déclaré directement dans le manifeste doit être pris en compte'
);

c=boot({developer:true});
assert.equal(c.getCanonical('app').mode,'live','un développeur DEV doit pouvoir exercer une application native avant certification');
assert.equal(c.getCanonical('app').freshInstallDeveloperPreview,true);
