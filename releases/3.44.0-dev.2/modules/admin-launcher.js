...
function add(){
 ...
 if(menu){
   const li=document.createElement("li"),a=document.createElement("a");a.id="kt-admin-link";a.href="#";a.className="dropdown-item";a.innerHTML=`<i class="${cfg.appearance?.icon||"fa fa-cog"}" aria-hidden="true"></i> ${cfg.label||"Pimp My Koha"}`;a.addEventListener("click",invoke);li.appendChild(a);menu.appendChild(li);return;
 }
 if(cfg.targets?.floatingFallback!==false){
   const b=document.createElement("button");b.id="kt-admin-link";b.type="button";b.textContent=cfg.label||"Pimp My Koha";b.style.cssText="position:fixed;right:12px;bottom:12px;z-index:2147480000;border:1px solid #1f6fb2;background:#1f6fb2;color:#fff;border-radius:7px;padding:7px 10px;font-weight:700;box-shadow:0 3px 14px rgba(0,0,0,.2)";b.addEventListener("click",invoke);document.body.appendChild(b)
 }
}
